...
// Heading uses clockwise degrees from true north; smoothed state is Q24.8.
static int32_t s_planet_seek_heading_target_deg = 0;
static int32_t s_planet_seek_heading_smoothed_fp = 0;
static int32_t s_planet_seek_heading_velocity_fp = 0;

static bool s_planet_seek_heading_has_reading = false;

// True until the compass reports a calibrated reading.
static bool s_input_planet_seek_compass_low_accuracy = true;

static void planet_seek_compass_handler(CompassHeadingData data) {
  // Pebble reports headings counter-clockwise; the app uses clockwise bearings.
...
static void planet_seek_heading_physics_step(void) {
  // Smooth toward the latest target at a fixed animation cadence.
...
#define COMPASS_FEATURE_FRAME_MS 300 // ~3 Hz compass refresh.
...
static void compass_feature_handler(CompassHeadingData data) {
  // Convert Pebble's counter-clockwise heading to the app's clockwise convention.
...
static void shake_anim_timer_callback(void *data) {
  // Keep one settling frame when the duration is crossed.
  (void)data;
