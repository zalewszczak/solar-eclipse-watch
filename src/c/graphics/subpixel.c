...
static uint8_t polygon_coverage9_fp(const FGPoint *pts, int n, int32_t center_x, int32_t center_y) {
  ...
  for (int oy = 0; oy < 3; oy++) {
    for (int ox = 0; ox < 3; ox++) {
      FGPoint sample = subpixel_fgpoint_new(center_x + SUBPIXEL_AA_OFFSETS3[ox], center_y + SUBPIXEL_AA_OFFSETS3[oy]);
      if (subpixel_point_in_convex_polygon_fp(pts, n, sample)) hits++;
    }
  }
  return hits;
}
...
void subpixel_stroke_line_fp(GContext *ctx, FGPoint a, FGPoint b, GColor color, bool dithered) {
  ...
  for (int i = 0; i <= steps; i++) {
    int16_t px = subpixel_fp_round_to_px(cur_x);
    int16_t py = subpixel_fp_round_to_px(cur_y);
...
void subpixel_stroke_polygon_fp(GContext *ctx, const FGPoint *pts, int n, GColor color, bool dithered) {
  for (int i = 0; i < n; i++) {
    subpixel_stroke_line_fp(ctx, pts[i], pts[(i + 1) % n], color, dithered);
  }
}

void subpixel_stroke_circle_fp(GContext *ctx, FGPoint center, int32_t radius_fp, GColor color, bool dithered) {
  int16_t r_px = subpixel_fp_round_to_px(radius_fp);
  if (r_px < 1) r_px = 1;

  graphics_context_set_fill_color(ctx, color);
  int16_t x = r_px, y = 0, err = 0;
  int16_t cx = subpixel_fp_round_to_px(center.x);
  int16_t cy = subpixel_fp_round_to_px(center.y);
...
bool subpixel_inset_convex_polygon_fp(const FGPoint *pts, int n, int32_t d_fp, FGPoint *out_pts) {
  ...
  FGPoint centroid = subpixel_fgpoint_new(0, 0);
  ...
  for (int i = 0; i < n; i++) {
    ...
    offset_a[i] = subpixel_fgpoint_new(a.x + off_x, a.y + off_y);
...
void subpixel_fill_polygon_ring_fp(GContext *ctx, const FGPoint *pts, int n, int32_t thickness_fp, GColor color, bool dithered) {
  ...
  bool have_inner = subpixel_inset_convex_polygon_fp(pts, n, thickness_fp, inner);

  if (!have_inner) {
    if (dithered) subpixel_fill_polygon_dithered_fp(ctx, pts, n, color);
    else subpixel_fill_polygon_fp(ctx, pts, n, color);
    return;
  }
  ...
  for (int16_t y = min_y; y <= max_y; y++) {
    ...
    for (int16_t x = min_x; x <= max_x; x++) {
      ...
      int32_t sample_x = ((int32_t)x << SUBPIXEL_BITS) + SUBPIXEL_HALF;
      FGPoint sample = subpixel_fgpoint_new(sample_x, sample_y);
      if (subpixel_point_in_convex_polygon_fp(pts, n, sample) && !subpixel_point_in_convex_polygon_fp(inner, n, sample)) {
        graphics_fill_rect(ctx, GRect(x, y, 1, 1), 0, GCornerNone);
      }
    }
  }
...
void subpixel_fill_circle_ring_fp(GContext *ctx, FGPoint center, int32_t outer_r_fp, int32_t thickness_fp, GColor color, bool dithered) {
  ...
  if (inner_r_fp <= 0) {
    subpixel_fill_circle_fp(ctx, center, outer_r_fp, color, dithered);
    return;
  }
